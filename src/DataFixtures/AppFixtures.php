...
//            }
    ...
    protected function createUsers(ObjectManager $manager, array &$users, UserPasswordHasherInterface $hasher): void
    {
        for ($i = 0; $i < self::MAX_USERS; $i++) {
            $user = new User();
            $user->setEmail(email: "test_$i@example.com");
            $user->setUsername(username: "test_$i");
            $user->setPassword(password: $hasher->hashPassword($user, 'coucou', ));
